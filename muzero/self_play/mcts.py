...
def select_child(config: MuZeroConfig, node: Node, min_max_stats: MinMaxStats):
    ...
    '''
    max_ucb = max(ucb_score(config, node, child, min_max_stats) for action, child in node.children.items())
    action = numpy.random.choice([action for action, child in node.children.items() if ucb_score(config, node, child, min_max_stats) == max_ucb])
