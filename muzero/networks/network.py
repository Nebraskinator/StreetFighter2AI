...
class NetworkOutput(typing.NamedTuple):
    value: float
    reward: float
    policy_softmax: typing.Optional[List[float]]
    ...
    def build_softmax_policy(softmax_logits):
        return {Action(i): logit for i, logit in enumerate(softmax_logits)}
...
class UniformNetwork(AbstractNetwork):
    ...
    def initial_inference(self, image) -> NetworkOutput:
        return NetworkOutput(0, 0, {Action(i): 1 / self.action_size for i in range(self.action_size)}, {Action(i): 1 / self.action_size for i in range(self.action_size)}, None)
...
class InitialModel(Model):
    ...
    def __init__(self, representation_network: Model, value_network: Model, policy_network: Model, exploration_network: Model):
    ...
    def call(self, image):
        ...
        exploration_logits = self.exploration_network(hidden_representation)
        return hidden_representation, value, policy_logits, exploration_logits
...
class RecurrentModel(Model):
    ...
    def call(self, conditioned_hidden):
        ...
        policy_logits = self.policy_network(hidden_representation)
        exploration_logits = self.exploration_network(hidden_representation)
        return hidden_representation, reward, value, policy_logits, exploration_logits
...
class BaseNetwork(AbstractNetwork):
    ...
    def __init__(self, representation_network: Model, value_network: Model, policy_network: Model,
                 dynamic_network: Model, reward_network: Model, exploration_network: Model):
        super().__init__()
        # Networks blocks
        self.representation_network = representation_network
        self.value_network = value_network
        ...
        self.initial_model = InitialModel(self.representation_network, self.value_network, self.policy_network, self.exploration_network)
        self.recurrent_model = RecurrentModel(self.dynamic_network, self.reward_network, self.value_network,
                                              self.policy_network, self.exploration_network)

    def initial_inference(self, image: np.array) -> NetworkOutput:
        """representation + prediction function"""

        hidden_representation, value, policy_logits, exploration_logits = self.initial_model.predict(np.expand_dims(image, 0))
        output = NetworkOutput(value=self._value_transform(value),
                               reward=0.,
                               policy_softmax = NetworkOutput.build_softmax_policy(self._softmax(policy_logits[0])),
    ...
    def recurrent_inference(self, hidden_state: np.array, action: Action) -> NetworkOutput:
        ...
        hidden_representation, reward, value, policy_logits, exploration_logits = self.recurrent_model.predict(conditioned_hidden)
        output = NetworkOutput(value=self._value_transform(value),
    ...
    @abstractmethod
    def _softmax(self, value: np.array) -> np.array:
        pass    
    ...
    def ex_get_variables(self) -> Callable:
        """Return a callback that return the trainable variables of the network."""

        def get_variables():
            networks = (self.exploration_network,)
            return [variables
                    for variables_list in map(lambda n: n.weights, networks)
                    for variables in variables_list]
